...
import { handleDirectRequest as handleDirectRequestWithOptions } from "./direct-request-handler.js";
...
export async function handleDirectRequest(
  optionsOrContext: HandleDirectRequestOptions | Context,
  upstream?: UpstreamAdapter,
  req?: ProxyRequest,
  fmt?: FormatAdapter,
): Promise<Response> {
  const options: HandleDirectRequestOptions =
    "upstream" in optionsOrContext
      ? optionsOrContext
      : {
          c: optionsOrContext,
          upstream: upstream!,
  ...
  return handleDirectRequestWithOptions(options);
}
...
export async function handleProxyRequest(
  optionsOrContext: HandleProxyRequestOptions | Context,
  accountPoolArg?: AccountPool,
  cookieJarArg?: CookieJar,
  reqArg?: ProxyRequest,
  fmtArg?: FormatAdapter,
  proxyPoolArg?: ProxyPool,
): Promise<Response> {
  const options: HandleProxyRequestOptions =
    "accountPool" in optionsOrContext
      ? optionsOrContext
      : {
          c: optionsOrContext,
          accountPool: accountPoolArg!,
          cookieJar: cookieJarArg,
          req: reqArg!,
          fmt: fmtArg!,
          proxyPool: proxyPoolArg,
        };
