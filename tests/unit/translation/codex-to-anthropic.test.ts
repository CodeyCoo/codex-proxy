...
describe("streamCodexToAnthropic", () => {
  ...
  it("does NOT emit thinking blocks (proxy cannot produce valid signature)", async () => {
...
describe("collectCodexToAnthropicResponse", () => {
  ...
  it("does NOT include thinking block even when requested", async () => {
...
describe("streamCodexToAnthropic — thinking block ordering", () => {
  it("does NOT emit thinking block even when wantThinking is true", async () => {
    const chunks = await collectStreamOutput(reasoningStream(), true);
    const events = parseSSEEvents(chunks);
    const blockStarts = events.filter((e) => e.event === "content_block_start");
    // Thinking is intentionally dropped — proxy cannot produce valid signature
    const thinkingStarts = blockStarts.filter(
      (e) => (e.data.content_block as Record<string, unknown>)?.type === "thinking",
    );
    expect(thinkingStarts).toHaveLength(0);
    expect((blockStarts[0].data.content_block as Record<string, unknown>)?.type).toBe("text");
  });
});
