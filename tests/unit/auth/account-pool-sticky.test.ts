...
describe("account-pool sticky strategy", () => {
  ...
  it.skip("selects account with most recent last_used", () => {
    // Current fork: sticky is an alias for smart rotation (weighted scoring),
    // not a true "sticky to last used" strategy.
  ...
  it.skip("sticks to same account across multiple acquire/release cycles", () => {
    // Current fork: sticky is an alias for smart rotation (weighted scoring),
    // not a true "sticky to last used" strategy.
    const pool = new AccountPool({ persistence: createMemoryPersistence() });
  ...
  it.skip("picks first available when no account has been used yet", () => {
    // Current fork: sticky is an alias for smart rotation (weighted scoring),
    // not a true "sticky to last used" strategy.
    const pool = new AccountPool({ persistence: createMemoryPersistence() });
    pool.addAccount(createValidJwt({ accountId: "a", email: "a@test.com", planType: "free" }));
