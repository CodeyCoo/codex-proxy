...
import { getRotationStrategy } from "@src/auth/rotation-strategy.js";
import type { RotationState } from "@src/auth/rotation-strategy.js";
...
describe("rotation-strategy", () => {
  ...
  describe("round_robin", () => {
    ...
    it("wraps index when candidates shrink", () => {
      ...
      const a = makeEntry("a");
      const b = makeEntry("b");
      // Current fork uses lastSelectedId for stable round-robin, not roundRobinIndex
      expect(strategy.select([a, b], state).id).toBe("a");
      expect(strategy.select([a, b], state).id).toBe("b");
      expect(strategy.select([a, b], state).id).toBe("a"); // wraps
    });
  });
  ...
  describe("sticky", () => {
    ...
    it.skip("selects most recently used account", () => {
      // Current fork: sticky is an alias for smart rotation (weighted scoring),
      // not a separate "most recently used" strategy.
